  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <h3 className="font-heading font-bold text-black">Notifications</h3>
        <button
          onClick={markAllRead}
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <div className="text-center py-8 text-[#666666]">No notifications</div>
        ) : (
          notifications.map((n) => (
            <button
              key={n._id}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-black leading-snug">{n.message}</p>
                <p className="text-xs text-[#666666] mt-1">{timeAgo(new Date(n.createdAt))}</p>
              </div>
              {!n.read && (
                <span className="w-2.5 h-2.5 bg-black rounded-full flex-shrink-0 mt-1.5" />
              )}
            </button>
          ))
        )}
